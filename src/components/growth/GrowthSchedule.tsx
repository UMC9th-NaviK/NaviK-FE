import { useMemo, useState } from "react"; // useState 추가
...
const GrowthSchedule = ({ logs } : GrowthScheduleProps) => {
    ...
    return (
        <div 
            ...
            className={`flex flex-col rounded-[16px] p-[16px] gap-[10px] border-[1px] cursor-pointer transition-all ${
                status === "COMPLETED" ? "border-primary-blue-100" : "border-primary-blue-500"
            }`}
        >
            <div className={`flex flex-col gap-[5px] ${status === "COMPLETED" ? "opacity-50" : "opacity-100"}`}>
                ...
                <div className='flex flex-col gap-[2px]'>
                    <p className={`text-body-16B text-primary-blue-500 ${isExpanded ? "" : "truncate"}`}> 
                        {logs.title} 
                    </p>
                    <p className={`text-body-14M text-[#111111CC] ${isExpanded ? "" : "truncate"}`}> 
                        {logs.content} 
                    </p>
                </div>
            </div>
